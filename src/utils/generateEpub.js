...
// ─── 블로그 붙여넣기용 HTML (티스토리 등) ─────────────────────────
// 블로그 에디터가 자체 CSS로 간격/폰트를 덮어쓰는 경우가 많아 !important 로 강제
export function messagesToBlogHtml(messages, includeSadam, bodyFont = DEFAULT_BODY_FONT) {
  const fontStack = BODY_FONTS[bodyFont] || BODY_FONTS[DEFAULT_BODY_FONT]
  const W = 'trpg-blog'
